...
const char * const MCUProjectNature = "fr.ac6.mcu.ide.core.MCUProjectNature";
const char * const MCUSingleCoreProjectNature = "fr.ac6.mcu.ide.core.MCUSingleCoreProjectNature";
...
typedef union {
    struct {
        ...
        uint32_t MCUSingleCoreProjectNature : 1;    // 9
    } bits;
    uint32_t dword;
} project_has_t;
    ...
    if (nodeType == XML_NODE_TYPE_START_ELEMENT) {
        ...
        switch (project_state[depth-1]) {
            ...
            case PROJECT_STATE_NATURE:
                ...
                else if (strcmp(MCUSingleCoreProjectNature, value) == 0)
                    project_has.bits.MCUSingleCoreProjectNature = 1;
                else {

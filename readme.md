...
Test utility ``cdt-xml-compare`` permits testing via comparing a control reference file against another file.``cdt-xml-compare`` compares ``.cproject`` files and ``.project`` files.  The intent is to find errors in eclipse XML files which would prevent them from being recognized as valid project by IDE.  When project file has error, typically the IDE will only say it cant read it, or just crash rather than give the actual problem.  This compare utility is intented to show where the difference is between a project file created by an IDE (control file), vs. a file created by something else (file unter test).  Often the file under test will have elements which do not exist in control file; they are printed out in the end of test. ``cdt-xml-compare`` is built, but not installed.
